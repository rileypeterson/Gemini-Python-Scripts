...
### Logistics
base_folder = "/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/simulations/" #directory where simulations will be made, created if it doens't exist
login_cl_path = "/Users/rpeterson/" #so that from pyraf import iraf is imported properly
psf_image = "/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/psf-1500-1500.fits" #psf kernel that will be convolve with the galaxies
...
large_field_xdim,large_field_ydim = 3133,3038  # x,y ; the dimensions of your simulated image
...
number_of_sims = 800 # Should be even int, number of simulated galaxies in the image, if make_pairs="yes" and the maximum pixel distance is large it might not be possible to place this number, the script will inform the user if this happens
...
universal_seed = 12192017 ### can be None or int, super helpful for reproducibility. Same seed produces same results
...
if make_interpolate_chain=="yes":
    ...
    chain_plot1 = "yes"
    chain_bins1 = 40
    chain_data1 = "/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/UV105842F160W_tab_all.fits" #Only works for fits files right now, might make for text later
    ...
    if do_chain2=="yes":
        ...
        chain_type2 = "linear"
        chain_plot2 = "yes"
        chain_data2 = "/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/UV105842F160W_tab_all.fits" #Only works for fits files right now, might make for text later
...
make_pairs="yes" #if running create_large_field and you want pairs this should stay "yes"
if make_pairs=="yes":
    pixel_distance_steps=np.linspace(2,40,40) #These can also be custom non linear arrays
    luminosity_ratio_steps=np.linspace(0.25,4.0,10) #2 means child galaxy will be twice as bright
...
if create_large_field=="yes":
    image_to_sim = "/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/UV105842F160W_drz.fits" #should be full path to it, shouldn't be in base_folder. If not "default", the image provided will be used to determine possible coordinates (useful if image is rotated)
    ...
    if add_models == "yes":
        ...
        how_many_to_add = 800 # should be <= number_of_sims, number of models that will be added to the large field
        ...
        add_mknoise_large2="no" #If drizzling this should be "no", but the values below should be defined. if "yes", then drizzle should be set to "no", and noise is added using IRAF's mknoise.
        background2 = 0.623
        gain2=1632.34375/2.5 #mknoise gain, here I use the exptime of the flt images as the gain 652.9375, when I add 45 it more closely reproduces noise, but this is unjustified so I stopped this practice.
        rdnoise2=20 #mknoise read noise
        poisson2="yes" #mknoise add poisson noise
            
        drizzle="yes" #if "yes", drizzling is performed
        if drizzle=="yes":
            path_to_flts="/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/id6p01???_flt.fits" #Path to your flt images wildcard (*) or question mark (?) should be placed where necessary. E.g. path_to_flts="/Users/rpeterson/Downloads/id6p01???_flt.fits", such that glob.glob(path_to_flts) will get all your flts
            path_to_actual_driz="/Users/rpeterson/FP/GALFIT_Simulations1/galfitsim1_python_scripts/most_current/examples/example_1/UV105842F160W_drz.fits"
            display_blots="yes" #Will display blotted (distorted flts)
            backgrounds=[0.6361,0.607,0.6383,0.607] #If "default" will use the background2 value above as the value for each flt, otherwise it is the list of specific values for each simulated flt e.g. backgrounds=[0.6351,0.606,0.6373,0.606]
...
def comp1(file1,cutout_size,mag_range,re_range,n_range,q_range,pa_range,plt_scale_ratio=1):
    ...
    file1.write(" 3) "+str(random.uniform(mag_range[0],mag_range[1]))+"     1          #  total magnitude"+'\n')
    file1.write(" 4) "+str(random.uniform(re_range[0],re_range[1])/plt_scale_ratio)+"      1          #      R_e [pixels]"+'\n')
    file1.write(" 5) "+str(random.uniform(n_range[0],n_range[1]))+"     1          #  exponent (de Vaucouleurs = 4)"+'\n')
    file1.write(" 9) "+str(random.uniform(q_range[0],q_range[1]))+"      1          #  axis ratio (b/a)"+'\n')
    file1.write("10) "+str(random.uniform(pa_range[0],pa_range[1]))+"     1          #  position angle (PA) [deg: Up=0, Left=90]"+'\n')
    file1.write('\n')
...
if make_feedmes=='yes':
    ...
    for i in range(1,number_of_sims+1):
        ...
        if make_interpolate_chain=="yes":
            ...
            comp1(file1,cutout_size,mag_range,re_range,n_range,q_range,pa_range,plt_scale_ratio=1)
        sky(file1,sky_range)
