...
def plots_sim_stats(dataframe_from_make_stats_dict,plot_sext="yes",plot_gal="yes",plot_sext_misses="yes",plot_gal_misses="yes"):
    ...
    #if plot_gal_misses=="yes": df_galfit_misses=df_sext_misses.T.dropna().T.filter(df_sext_misses.columns)
    ...
    #if plot_gal=="yes": where.errorbar(df_galfit_misses.T.sim_mag.astype(float),df_galfit_misses.T.gal_mag-df_galfit_misses.T.sim_mag.astype(float),yerr=df_galfit_misses.T.gal_mag_err,fmt="s",c="b",marker="x",ms=4)

    #Magnitude errors against angular distance (True)
    where=l
    where.set_title("Magnitude Error for GALFIT against True Angular Distance");where.set_xlabel("Angular Distance (True) [pixels]");where.set_ylabel("Output Magnitude - Input Magnitude")
    ...
    where=br
    if plot_gal=="yes": where.set_title("magnitude error vs. logre error");where.set_xlabel("GALFIT log(re) - Input log(re)");where.set_ylabel("GALFIT Magnitude - Input Magnitude")
    ...
    if plot_gal=="yes": where.scatter(df_galfit.T.gal_logre-df_galfit.T.sim_logre.astype(float),df_galfit.T.gal_mag-df_galfit.T.sim_mag.astype(float),c="r",marker="x")
...
df=plots_sim_stats(a,plot_sext="no",plot_sext_misses="no")
