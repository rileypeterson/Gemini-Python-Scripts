...
make_ncomb='yes' #if "yes", makes the ncomb image. The pixel values in this image represent the number of stacked images which contributed to said pixel.
make_weight='yes' #if "yes", creates the weight image. If a pixel in the ncomb image has a value >= nmin, it keeps the corresponding value in the original wht image. Otherwise, the value is changed to zero.
make_sigma='yes' #if "yes", creates the sigma image. Formula for this is in the manual.
run_sex1='yes'  #if "yes", runs SExtractor. Outputs the table of information whose columns are the parameters listed in the param file. Also outputs the aperture, segmentation, and background image.
...
make_assoc='yes' #if "yes", creates an association file which distinguishes target galaxies. Galaxies are selected based on MAG_AUTO, MAG_AUTO_ERR, and CLASS_STAR from SExtractor.
...
do_run_galfit="no" #if "yes", the GALFIT tasks below can be executed
if do_run_galfit=="yes":
    ...
    run_refits_sersic1_list=[9] #Should NOT be "default", this is the list of galaxies that need to be refit
...
base_folder = "/Users/rpeterson/FP/GALFIT_Simulations1/testing_do_everything/test"  #Should not end with "/", CANNOT CONTAIN "_" AFTER THE LAST SLASH, I.E. BASE_NAME CANNOT NOT HAVE AN UNDERSCORE (IRONY). Now these syntactical nuances are corrected for by the script, so its not a huge deal.
drz_image = "test_drz.fits"  #Should be like this UV108899id6p03F160W_drz.fits, I might implement this automatically in the future, but then I would need to guess which image is which (probably by looking for wht or drz or ctx in the name)
wht_image = "test_wht.fits"  #Should be like this UV108899id6p03F160W_wht.fits
ctx_image = "test_ctx.fits"  #Should be like this UV108899id6p03F160W_ctx.fits

### Mkncomb, mkweight, sigma image inputs
n_min = 3 #For mkweight minimum number of contributing stacked images for a valid pixel
read_noise = 2.0 #For sigma image creation, read noise
...
N_GEOM_LEVELS = "1.0"
N_MOFFAT_BETA = "1.0"
PARAMETERS_NAME = "/Users/rpeterson/FP/GALFIT_Simulations1/testing_do_everything/test/MSassocAP.param"
...
assoc_mag_cut = 24.5 ### MAG_AUTO cut for association file
assoc_noise_cut = 0.04 ### MAG_AUTO_ERR cut for association file remember MAG_ERR=0.04 corresponds to S/N~25 (Ryan Cole)
assoc_class_star_cut = 1 ### CLASS_STAR cut for association file

### Mask inputs
mask_maglim = 25.5 ### maglim = min(magbright+6.2,magfaint+0.5), very important, objects fainter than this will be masked
...
take_average_list=[] #list of ids whose sky value should be the average of the iterstat and sextractor value
