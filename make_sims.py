...
def cfig(config, cfig_section, cfig_key,
    ...
    """
    try:
        if config.get(cfig_section, cfig_key) == "None":
            return
...
def first_interpolation(input_fits, fits_column, interp_param,
    ...
    fits_column : string
        Name of the to be interpolated column within the .fits file
    interp_param : string
        Name of the parameter to be interpolated ('mag', 're', 'n', etc.)
    bins : int, default 40
    ...
    """
    # Open the input_data and form a histogram based on bins
    data_array = fits.open(input_fits)[1].data[fits_column]
    data_hist, data_bins = np.histogram(np.sort(data_array), bins=bins, normed=True, density=True)

    # Interpolate this histogram ("PDF")
    x_vals = np.linspace(np.min(data_array), np.max(data_array), len(data_hist))
    interp_func = interpolate.interp1d(x_vals, data_hist*np.diff(data_bins),
                                       kind=interp_type, fill_value="extrapolate")

    # Resample very finely, potentially an issue if range of data is large
    fine_x_vals = np.linspace(np.min(data_array), np.max(data_array), 100000)
    weights = interp_func(fine_x_vals)/np.sum(interp_func(fine_x_vals))
    weights[weights < 0] = 0

    # Select from this array with appropriate weight
    ret_array = np.random.choice(fine_x_vals, size=number_of_sims, p=weights/np.sum(weights))
    chain_sim_data = np.random.choice(fine_x_vals, size=100000, p=weights/np.sum(weights))
    ...
    plt.title("First Interpolation Results")
    # TODO plt.savefig()
...
class Galaxy(object):
    """ Object which represents a galaxy. """
...
def input_top(file_path, output_name, mag_zp, cutout_size=400):
    """
    Write the top of the input file for each galaxy.

    Parameters
    ----------
    file_path : str
        Path where the input file is to be written to
    output_name : str
        File name of the fits file that will be produced when the input file is ran through GALFIT
    mag_zp : float
        Magnitude photometric zeropoint
    cutout_size : int
        Size in pixels of the cutout. Cutouts are square each edge being the length {cutout_size}
...
def comp1(file1, mag, re, n, q, pa, cutout_size=400):
    """
    Write the first component for each galaxy.

    Parameters
    ----------
    file1 : file object
        file object to be written to, typically passed from input_top
    galaxy : Galaxy object
        a galaxy object which has attributes like mag, re, etc.


    Returns
    -------
    Nothing. It writes to the designated file object.
    ...
    file1.write(" 3) {mag}     1          #  total magnitude".format(mag=mag)+'\n')
    file1.write(" 4) {re}      1          #      R_e [pixels]".format(re=re)+'\n')
    file1.write(" 5) {n}     1          #  exponent (de Vaucouleurs = 4)".format(n=n)+'\n')
    file1.write(" 9) {q}      1          #  axis ratio (b/a)".format(q=q)+'\n')
    file1.write("10) {pa}     1          #  position angle (PA) [deg: Up=0, Left=90]".format(pa=pa)+'\n')
    file1.write('\n')
...
def sky(file1, sky_range):
    """
    Write the sky component for each galaxy.

    Parameters
    ----------
    file1 : file object
        file object to be written to, typically passed from input_top
    sky_range : list or tuple (length 2)
        lower and upper bound for the sky, fallback is [0, 0]


    Returns
    -------
    Nothing. It writes to the designated file object and closes it.
...
def main():
    ...
    base_folder = cfig(config, "File_Paths","base_folder")
    if not base_folder:
    ...
    if not os.path.exists(base_folder):
        ...
        os.mkdir(base_folder)
    return config, logger, base_folder
    ...
    if cfig(config, "Tasks", "make_feedmes") == "yes":
        ...
        if feedme_method == "interpolate":
            ...
            interp_vals1 = first_interpolation(cfig(config, s, "input_fits"),
                                               ...
                                               number_of_sims=ns)
            interp_dict.update({cfig(config, s, "interp_param1", fallback="mag"):list(interp_vals1)})
            if cfig(config, s, "do_interp2") == "yes":
                interp_func2 = second_interpolation(cfig(config, s, "input_fits"),
                                                    cfig(config, s, "fits_column1", fallback="MAG_AUTO"),
                                                    cfig(config, s, "fits_column2"),
                                                    cfig(config, s, "interp_param1", fallback="mag"),
                                                    cfig(config, s, "interp_param2"),
                                                    interp_type=cfig(config, s, "interp_type2", fallback="linear"),
                                                    number_of_sims=ns)
                interp_vals2 = list(interp_func2(interp_vals1))
                interp_dict.update({cfig(config, s,"interp_param2"):interp_vals2})
            # TODO create generate_feedme function should default to random
            gals_dict = create_galaxies(ns, config, interp_dict)
